    ...
    <script type="text/javascript">
      $(document).ready(function() {
        var dt = $('#users').DataTable({
          "processing": true,
          "serverSide": true,
          "ajax": { "url": 'sms_per_user_alt_db.php' },
          "columns": [
            {
        ...
        function format (callback, d ) {
          ...
          $.ajax({
            url: "select_details_per_user_db.php",
            method: "POST",
            dataType: "json",
            data: {
              Username: d.Username
            ...
            complete: function (response) {
              ...
              var thead = '<thead id="created">',  tbody = '';
              if (!( data[0] == undefined )){
                ...
                $.each(data, function (i, d) {
                  tbody += '<tr><td>'+d.num+ '</td><td>'+ d.Username + '</td><td>' + d.surname + '</td>\
...
<td>' + d.smsid + '</td><td>' + d.remarks + '</td><td>' + d.status + '</td>\
              ...
              callback($('<table id="mytest" class="table table-bordered">' + thead + tbody + '</table>')).show();
            $('#mytest').DataTable();
            }
            ,  error: function () {
